...
public class SeedController : ControllerBase
{
    ...
    public async Task<IActionResult> Put()
    {
        ...
        var allTagNames = new HashSet<string>(StringComparer.InvariantCultureIgnoreCase);
        foreach (var record in records)
        {
            if (!string.IsNullOrEmpty(record.Tags))
            {
                ...
                {
                    if (!string.IsNullOrEmpty(tagName) && !existingTagsDict.ContainsKey(tagName))
                    {
        ...
        using (var transaction = _context.Database.BeginTransaction())
        {
            try
            {
                _context.Database.ExecuteSqlRaw("SET IDENTITY_INSERT Courses ON");
                ...
                foreach (var record in records)
                {
                    ...
                    {
                        _logger.LogError(ex, "Error inserting course: {CourseId}, {Title}", 
                            record.Id, record.Title);
                    }
                }
                
                _context.Database.ExecuteSqlRaw("SET IDENTITY_INSERT Courses OFF");
                transaction.Commit();
            }
            catch (Exception ex)
            {
        ...
        foreach (var record in records)
        {
                ...
                .Distinct(StringComparer.InvariantCultureIgnoreCase))
            {
                ...
                {
                    _logger.LogError(ex, "Error inserting course-tag relationship: CourseId={CourseId}, TagName={TagName}",
